@echo off
REM ============================================
REM Daily Market Report - PM Edition COMPLETO
REM Ejecuta a las 18:45 (6:45 PM) <- NUEVO HORARIO
...
echo [%date% %time%] Iniciando proceso PM COMPLETO >> logs\report_log.txt
echo ================================================ >> logs\report_log.txt

echo.
echo ========================================
echo GREY BARK ADVISORS - REPORTE PM
echo ========================================
...
echo       Incluye cierre USA del dia (NYSE 18:00 Chile)
echo.
python daily_market_snapshot.py PM >> logs\snapshot_PM.log 2>&1
...
if %errorlevel% neq 0 (
    ...
    echo Ver logs\snapshot_PM.log para detalles
    echo ========================================
    pause
    exit /b 1
)
...
echo.
python generate_daily_report.py PM >> logs\generate_PM.log 2>&1
...
if %errorlevel% neq 0 (
    ...
    echo Ver logs\generate_PM.log para detalles
    echo ========================================
    pause
    exit /b 1
)
...
echo.
python html_formatter.py >> logs\html_formatter_PM.log 2>&1
...
if %errorlevel% neq 0 (
    ...
    echo Ver logs\html_formatter_PM.log para detalles
    echo ========================================
    pause
    exit /b 1
)
...
echo.
python send_email_v2.py auto >> logs\email_PM.log 2>&1

if %errorlevel% neq 0 (
    echo [%date% %time%] ADVERTENCIA: Fallo al enviar emails >> logs\report_log.txt
    echo       [WARN] Error al enviar emails (ver logs\email_PM.log)
) else (
    echo [%date% %time%] Emails enviados OK >> logs\report_log.txt
    echo       [OK] Reportes PM enviados a destinatarios
)
...
REM ============================================
echo.
echo [%date% %time%] Proceso PM completado >> logs\report_log.txt
echo. >> logs\report_log.txt

echo ========================================
echo PROCESO PM COMPLETADO
echo ========================================
echo Fin: %date% %time%
echo.
echo Archivos generados:
for %%f in (daily_report_PM_*.md) do (
    echo   MD:   %%f
)
for %%f in (html_out\daily_report_PM_*.html) do (
    echo   HTML: %%f
)
echo.
