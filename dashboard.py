...
# =========================================================================
# TOP NAVIGATION (replaces sidebar for reliability)
# =========================================================================

# Header bar with company name and navigation
_nav_col1, _nav_col2 = st.columns([2, 1])
with _nav_col1:
    company_name = THEME['company_name']
    st.markdown(f'<div class="main-header">{company_name}</div>', unsafe_allow_html=True)
with _nav_col2:
    _nav_right_cols = st.columns([3, 1])
    with _nav_right_cols[0]:
        st.markdown(f"*{datetime.now().strftime('%d %b %Y %H:%M')}*")
    with _nav_right_cols[1]:
        if st.button("Salir"):
...
_s1.metric("Research", f"{len(research_files)} archivos")
_s2.metric("Directivas", "Si" if directives else "No")
_s3.metric("Council Runs", len(recent_results))
if CLIENT_CONFIG and CLIENT_CONFIG.get('usage'):
    _s4.metric("AI Council (mes)", CLIENT_CONFIG['usage'].get('ai_council_count', 0))
else:
    _s4.metric("AI Council (mes)", 0)
