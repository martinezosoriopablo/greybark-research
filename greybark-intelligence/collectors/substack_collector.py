# -*- coding: utf-8 -*-
"""
SUBSTACK COLLECTOR - Recolecta artículos de Substack vía RSS feeds.
Usa feedparser para parsear los feeds públicos de cada Substack.
"""
...
class SubstackCollector(BaseCollector):
    """Recolecta artículos de Substack vía feeds RSS."""

    def __init__(self, config_path: str = None, hours_back: int = 24):
        super().__init__(source_type="substack", hours_back=hours_back)
    ...
    def _load_sources(self) -> List[Dict[str, Any]]:
        """Carga la lista de Substacks desde sources.yaml."""
        try:
            with open(self.config_path, 'r', encoding='utf-8') as f:
                config = yaml.safe_load(f)
            sources = config.get('substacks', [])
            self.log("INFO", f"Cargadas {len(sources)} fuentes Substack")
            return sources
    ...
    def collect(self) -> List[Dict[str, Any]]:
        """Recolecta artículos de todos los feeds Substack configurados."""
        items = []
        ...
        for source in self.sources:
            ...
            try:
                ...
                for entry in feed.entries:
                    ...
                    # Extraer contenido: preferir summary (resumen del RSS)
                    content = getattr(entry, 'summary', '') or ''
                    if not content:
                        # Fallback a content si existe
        ...
        self.log("INFO", f"Total Substack: {len(items)} artículos recolectados")
        return items
...
if __name__ == "__main__":
    import json
    print("=" * 60)
    print("SUBSTACK COLLECTOR - Test independiente")
    print("=" * 60)

    # Para testing: ampliar ventana a 7 días para ver resultados
    collector = SubstackCollector(hours_back=168)
    ...
    output_path = os.path.join(os.path.dirname(__file__), "..", "data", "test_substack.json")
