...
class CorrelationMatrix(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        lines = [
            "[CORRELATION MATRIX MODULE — Matriz de Correlaciones (mide cómo se mueven los activos entre sí en ventana móvil de 60 días; correlación alta = menor diversificación, rupturas = cambio de régimen)]",
            f"Correlación cruzada 60 días (promedio: {avg:.2f}, donde 1.0 = movimiento idéntico, 0 = independiente, -1.0 = opuesto)",
            f"Activos: {', '.join(self._result['labels'])}",
        ]

        if breaks:
            lines.append(f"\nRupturas de correlación (últimos {self.LOOKBACK_CHANGE} días) — cambios abruptos que señalan posible cambio de régimen:")
            for b in breaks:
                lines.append(
                    f"  {b['asset_1']} / {b['asset_2']}: "
                    f"{b['corr_before']} -> {b['corr_now']} (cambio: {b['change']:+.2f}) *** RUPTURA ***"
                )
        else:
            lines.append("\nSin rupturas significativas de correlación.")

        if stress:
            lines.append("\nSeñales de estrés (pares históricamente no correlacionados ahora alineados — típico en crisis):")
            for s in stress:
                lines.append(
                    f"  {s['asset_1']} / {s['asset_2']}: "
                    f"promedio histórico {s['avg_corr']:.2f}, actual {s['corr_now']:.2f}"
                )
