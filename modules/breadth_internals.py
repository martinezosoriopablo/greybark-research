...
class BreadthInternals(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        lines = [
            "[MARKET BREADTH MODULE — Amplitud de Mercado (mide qué porcentaje de acciones/sectores participan en la tendencia del mercado; un mercado sano sube con amplia participación)]",
            f"Score de Amplitud: {r['breadth_score']:.0f}/100 ({r['zone']})",
            f"Sectores sobre su media de 50 días (50MA): {n_above}/{total} ({pct:.1f}%)",
            f"Apetito por riesgo: {ra_label} (cíclicos {cyc_avg:+.1f}% vs defensivos {def_avg:+.1f}%)",
            f"Posición en el ciclo: {cycle_pos}",
            f"{sf_label}",
        ]

        if top:
            top_str = ", ".join(f"{etf} {ret:+.1f}%" for etf, ret in top)
            lines.append(f"Sectores líderes: {top_str}")
        if lag:
            lag_str = ", ".join(f"{etf} {ret:+.1f}%" for etf, ret in lag)
            lines.append(f"Sectores rezagados: {lag_str}")
