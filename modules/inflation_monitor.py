...
class InflationMonitor(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        lines = [
            "[INFLATION MONITOR MODULE — Monitor de Inflación (descompone la inflación en sus componentes para identificar presiones persistentes vs transitorias; incluye tasas reales y expectativas implícitas del mercado)]",
            f"Régimen de inflación: {r['inflation_regime']}",
            "Descomposición del CPI (Índice de Precios al Consumidor, EE.UU.):",
            f"  Todos los ítems: {_fmt(r.get('cpi_all'))} interanual (YoY)",
            f"  Core (subyacente, excluye alimentos y energía): {_fmt(r.get('cpi_core'))}{prev_str}",
            f"  Servicios: {_fmt(r.get('cpi_services'))} ({r.get('services_status', 'N/A')})",
            f"  Bienes: {_fmt(r.get('cpi_goods'))} ({r.get('goods_status', 'N/A')})",
            f"  Vivienda (Shelter): {_fmt(r.get('cpi_shelter'))}",
            f"  Brecha Servicios-Bienes: {r.get('services_goods_spread', 0):.1f}pp (puntos porcentuales)",
            f"Tasas reales: TIPS 10Y (bonos protegidos contra inflación) = {_fmt2(r.get('tips_10y'))} ({r['real_rate_regime']}, percentil {r.get('rr_percentile', 0):.0f})",
            f"Breakevens (inflación implícita del mercado): 5Y={_fmt2(r.get('be_5y'))}, 10Y={_fmt2(r.get('be_10y'))}, 5Y5Y fwd={_fmt2(r.get('be_forward_5y5y'))} ({r.get('be_status', 'N/A')})",
        ]

        if r.get('wage_yoy') is not None:
            lines.append(
                f"Riesgo de espiral salarios-precios: {r['wage_spiral_risk']} "
                f"(salarios {r['wage_yoy']:.1f}% vs CPI {r.get('wage_cpi', 0):.1f}%)"
            )

        lines.append(f"Señal TIPS (bonos indexados a inflación): {r['tips_signal']}" +
