...
class NarrativeDivergence(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        r = self._result
        lines = [
            "[NARRATIVE DIVERGENCE MODULE — Divergencia Narrativa (compara lo que dice el consejo vs lo que muestran los datos de mercado; divergencia alta = las opiniones no coinciden con la realidad del mercado)]",
            f"Última sesión del consejo: {r['session_date']}",
            f"Score de divergencia: {r['divergence_pct']:.0f}% ({r['overall']})",
        ]

        critical = r.get('critical_divergences', [])
        if critical:
            lines.append("DIVERGENCIAS CRÍTICAS (la narrativa contradice los datos):")
            for key in critical:
                check = r['checks'].get(key, {})
                detail = check.get('detail', '')
                label_map = {
                    'regime_vs_vix': 'Régimen vs VIX (volatilidad implícita)',
                    'fed_vs_rates': 'Postura Fed vs tasas de mercado',
                    'chile_vs_ipsa': 'Posición Chile vs IPSA (bolsa chilena)',
                    'risk_vs_credit': 'Riesgo vs spreads crediticios',
                    'conviction_vs_equity': 'Convicción vs renta variable',
                }
                lines.append(f"  {label_map.get(key, key)}: {detail}")
        else:
            lines.append("Sin divergencias críticas detectadas.")

        # Also list mild divergences
        mild = [k for k, v in r['checks'].items() if v.get('score') == 1]
        if mild:
            lines.append("Divergencias leves:")
            for key in mild:
                check = r['checks'].get(key, {})
                detail = check.get('detail', '')
        ...
        if aligned:
            lines.append(f"Alineados (narrativa consistente con datos): {', '.join(aligned)}")
