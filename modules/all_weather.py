...
class AllWeatherRegime(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        q = self._result
        lines = [
            "[ALL WEATHER REGIME MODULE — Régimen All Weather (modelo de asignación para todo clima, basado en el marco de Ray Dalio: clasifica el entorno en 4 cuadrantes según crecimiento e inflación)]",
            f"Cuadrante actual: {q['quadrant']} - {q['quadrant_name']} ({q['quadrant_label']})",
            f"Confianza: {q['confidence']:.0f}%",
            "",
            f"Señales de crecimiento (dirección: {q['growth']['direction']}, score promedio: {q['growth']['score']:.2f}):",
        ]
        for key, s in q['growth']['components'].items():
            arrow = '+' if s['score'] > 0 else ('-' if s['score'] < 0 else '=')
            lines.append(f"  {s['label']} -> {arrow}{s['score']}")

        lines.append(f"\nSeñales de inflación (dirección: {q['inflation']['direction']}, score promedio: {q['inflation']['score']:.2f}):")
        for key, s in q['inflation']['components'].items():
            arrow = '+' if s['score'] > 0 else ('-' if s['score'] < 0 else '=')
            lines.append(f"  {s['label']} -> {arrow}{s['score']}")

        lines.append(f"\nPesos recomendados por el modelo:")
        for asset, w in q['model_weights'].items():
            lines.append(f"  {asset}: {w:.0%}")

        if q.get('transition'):
            t = q['transition']
            lines.append(f"\nTRANSICIÓN DE RÉGIMEN: {t['from_name']} -> {t['to_name']}")
        else:
            lines.append(f"\nTransición: Estable (mismo cuadrante)")

        if q.get('regime_comparison'):
            rc = q['regime_comparison']
            lines.append(f"\nComparación con clasificador de régimen: {rc['classification']} (score: {rc['score']:.2f})")

        lines.append(f"\nInterpretación: {q['interpretation']}")
        return "\n".join(lines)
