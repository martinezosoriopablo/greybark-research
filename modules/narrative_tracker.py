...
class NarrativeTracker(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        r = self._result
        lines = [
            "[NARRATIVE TRACKER MODULE — Rastreador de Narrativa (analiza la evolución de las posturas del consejo a lo largo de múltiples sesiones; detecta cambios de visión y consistencia)]",
            f"Sesiones analizadas: {r['n_sessions']} ({r['date_range']})",
        ...
        if recent:
            lines.append("Cambios recientes de postura:")
            for dim_key, shift in recent.items():
                short = dim_short.get(dim_key, dim_key)
                lines.append(f"  {short}: {shift['from']} -> {shift['to']} (desde {shift['date']})")
        else:
            lines.append("Cambios recientes: Ninguno (narrativa estable)")
