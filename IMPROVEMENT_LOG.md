...
| 205 | **Copper sensitivity Chile** — tabla de sensibilidad: "$0.25/lb cambio en cobre = X bps fiscal, Y% CLP, Z bps BCP". 5 escenarios (-$0.50 a +$0.50) con colores rojo/verde | `report_enhancements.py:generate_copper_sensitivity_html()`, AA renderer + template | Estándar LatAm research: cobre = ~50% exportaciones Chile |
| 207 | **Quant signal dashboard** — tabla compacta momentum/carry/value/vol por asset class con checkmarks verdes/rojas. Score agregado (3/4 = bullish). Overlay cualitativo del council. Alimentado por TAA tilts | `report_enhancements.py:generate_quant_signal_dashboard_html()`, AA renderer | JP Morgan: quant signals + qualitative overlay |
| 209 | **Traffic-light conviction grid** — tabla resumen con toda la visión del comité en 1 glance. Filas = asset classes, celdas coloreadas verde (OW) / amarillo (N) / rojo (UW) con estrellas de convicción. Header negro. Columnas táctico (1-3m) y estratégico (6-12m) | `report_enhancements.py:generate_traffic_light_grid_html()`, AA renderer + template | Goldman Sachs: 1-page house view |
| 210 | **Pull quote del CIO** — frase clave del council en formato editorial grande (14pt italic, borde naranja, fondo gradient). Extrae automáticamente la primera oración con "creemos/vemos/nuestra lectura" del final_recommendation | `report_enhancements.py:generate_pull_quote_html()`, AA renderer | Magazine editorial style |
| 212 | **Annotated charts** — 7 market events (COVID, Rusia-Ucrania, Fed +75bp, SVB, UST 5%, Fed corta, Irán-Hormuz) como líneas verticales con labels en charts time-series. `annotate_events()` static method en ChartGenerator base class. Integrado en: (1) RF yield curve 10Y panel, (2) Macro multi-panel charts (commodities, policy rates, etc.) — solo paneles >24 meses | `chart_generator.py:MARKET_EVENTS + annotate_events()`, `rf_chart_generator.py` | Goldman: never a bare chart, always contextual annotations |
| 211 | **Sparkline SVG** — mini gráfico de tendencia inline (60×16px) para tablas. Polyline SVG con punto final coloreado según dirección. Reutilizable en cualquier tabla | `report_enhancements.py:generate_sparkline_svg()` | Goldman: every data row has a trend |
| 208 | **Z-score table** — 6 indicadores macro (VIX, UST 10Y, IG spread, P/E, CPI core, TPM) con nivel actual, promedio 5Y, z-score. Valores extremos (>1.5σ) resaltados rojo/naranja | `report_enhancements.py:generate_zscore_table_html()`, AA renderer | Bridgewater: at-a-glance abnormality detection |
| 206 | **Callout boxes** — 4 tipos: Key Change (azul), Conviction Trade (verde), Risk Alert (rojo), Info (gris). CSS con borde izquierdo + background + icon + label. Reutilizables en cualquier renderer | `report_enhancements.py:callout_box()` | Goldman/JP Morgan: visual hierarchy for key information |
