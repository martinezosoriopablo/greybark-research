...
def fetch_returns(symbols: List[str], period: str = '5y') -> pd.DataFrame:
    """Fetch historical returns. Returns empty DataFrame on failure."""
    try:
...
def generate_risk_dashboard(weights: Dict[str, float], portfolio_value: float = 1000000) -> Dict:
    ...
    # Guard: if market data download failed, return degraded dashboard
    if returns.empty or len(returns) < 20:
        ...
        return {
            ...
            'data_status': 'degraded — market data download failed',
            'var': {'var_95_daily': None, 'var_99_daily': None, 'var_95_usd': None},
            'drawdown': {'current_drawdown': None, 'max_drawdown': None},
            'portfolio_stats': {'annualized_return': None, 'annualized_volatility': None, 'sharpe_ratio': None},
            'correlations': {'diversification_score': None},
            'stress_testing': {'scenarios': stress_tester.run_all_scenarios(), 'worst_case': stress_tester.worst_case_loss()},
            'liquidity': {'vix': vix_data, 'score': liquidity_score},
            'scorecard': {'overall_score': 5, 'status': 'UNKNOWN', 'factor_scores': {}},
        }
