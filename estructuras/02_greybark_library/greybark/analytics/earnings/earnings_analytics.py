...
class EarningsAnalytics:
    ...
    def get_balance_sheet_quality(self, symbol: str) -> Dict:
        """
        Analyze balance sheet quality from BALANCE_SHEET endpoint.

        Calculates: debt_to_equity, current_ratio, net_debt, working_capital_trend (QoQ).

        Args:
            symbol: Stock ticker

        Returns:
            Dict with metrics + quality_grade (A/B/C/D)
        """
        print(f"[Earnings] Fetching balance sheet quality for {symbol}...")
        ...
        total_debt = (self._safe_av_float(latest.get('shortTermDebt')) or 0) + \
                     (self._safe_av_float(latest.get('longTermDebt')) or 0)
        total_equity = self._safe_av_float(latest.get('totalShareholderEquity')) or 0
        current_assets = self._safe_av_float(latest.get('totalCurrentAssets')) or 0
        current_liabilities = self._safe_av_float(latest.get('totalCurrentLiabilities')) or 0
        cash = (self._safe_av_float(latest.get('cashAndCashEquivalentsAtCarryingValue')) or 0) + \
               (self._safe_av_float(latest.get('cashAndShortTermInvestments')) or 0)
        ...
        # Working capital trend (QoQ)
        wc_trend = None
        if len(quarterly) >= 2:
            prev = quarterly[1]
            prev_ca = self._safe_av_float(prev.get('totalCurrentAssets')) or 0
            prev_cl = self._safe_av_float(prev.get('totalCurrentLiabilities')) or 0
            prev_wc = prev_ca - prev_cl
            if prev_wc != 0:
                wc_trend = round(((working_capital - prev_wc) / abs(prev_wc)) * 100, 1)
        ...
        grade = 'A' if score >= 4 else ('B' if score >= 3 else ('C' if score >= 1 else 'D'))
        ...
        return {
            ...
            'working_capital_trend_pct': wc_trend,
            'quality_grade': grade,
        }
    ...
    def get_cash_flow_quality(self, symbol: str) -> Dict:
        """
        Analyze cash flow quality from CASH_FLOW endpoint.

        Calculates: FCF, FCF-to-net-income, capex intensity, FCF yield.
        ...
        Returns:
            Dict with metrics + quality_grade (A/B/C/D)
        """
        print(f"[Earnings] Fetching cash flow quality for {symbol}...")
        ...
        operating_cf = self._safe_av_float(latest.get('operatingCashflow')) or 0
        capex = abs(self._safe_av_float(latest.get('capitalExpenditures')) or 0)
        net_income = self._safe_av_float(latest.get('netIncome')) or 0

        fcf = operating_cf - capex

        fcf_to_ni = round(fcf / net_income, 2) if net_income != 0 else None

        # Capex intensity: need revenue from income statement (use OVERVIEW for market cap)
        # For capex_intensity we approximate with operating_cf as denominator
        capex_intensity = round(capex / operating_cf * 100, 1) if operating_cf > 0 else None

        # FCF yield: FCF / market_cap (annualized)
        fcf_yield_pct = None
        try:
            overview = self._make_request({'function': 'OVERVIEW', 'symbol': symbol})
            market_cap = self._safe_av_float(overview.get('MarketCapitalization'))
            if market_cap and market_cap > 0:
                fcf_annualized = fcf * 4  # Quarterly → annual
                fcf_yield_pct = round((fcf_annualized / market_cap) * 100, 2)
        except Exception:
            pass
        ...
        if fcf > 0:
            score += 1
        if fcf_to_ni is not None and fcf_to_ni > 1.2:
            score += 2
        elif fcf_to_ni is not None and fcf_to_ni > 0.8:
            score += 1
        if fcf_yield_pct is not None and fcf_yield_pct > 5:
            score += 2
        elif fcf_yield_pct is not None and fcf_yield_pct > 3:
            score += 1
    ...
    def get_earnings_quality_composite(
        self,
        symbol: str,
        earnings_hist: Dict,
        estimates: Dict,
        ...
        """
        Composite earnings quality score (0-100) from pre-downloaded data.

        NO API calls — receives data already fetched by caller.

        Components (25 pts each):
        - Beat rate: >75% = 25, >50% = 15, else 5
        - Revision trend: upgrade_pct_30d scaled linearly
        - FCF quality: fcf_to_ni >1.2 = 25, >0.8 = 15, else 5
        - Insider signal: BULLISH = 25, MIXED = 15, BEARISH = 5

        Args:
            symbol: Ticker
            earnings_hist: Result from get_earnings_history()
            estimates: Result from get_analyst_estimates()
            insider: Result from get_insider_activity()

        Returns:
            Dict with score (0-100), grade, components
        """
        components = {}

        # 1. Beat rate
        beat_rate = earnings_hist.get('track_record', {}).get('beat_rate_pct', 0) if 'error' not in earnings_hist else 0
        if beat_rate >= 75:
            components['beat_rate'] = 25
        elif beat_rate >= 50:
            components['beat_rate'] = 15
        else:
            components['beat_rate'] = 5

        # 2. Revision trend
        upgrade_pct = estimates.get('revision_summary', {}).get('upgrade_pct_30d', 50) if 'error' not in estimates else 50
        components['revision_trend'] = round(min(25, max(0, (upgrade_pct / 100) * 25)))

        # 3. FCF quality (placeholder — caller should enrich if available)
        # We check if earnings_hist has fcf_to_ni injected
        fcf_to_ni = earnings_hist.get('fcf_to_net_income')
        if fcf_to_ni is not None:
            if fcf_to_ni > 1.2:
                components['fcf_quality'] = 25
            elif fcf_to_ni > 0.8:
                components['fcf_quality'] = 15
            else:
                components['fcf_quality'] = 5
        else:
            components['fcf_quality'] = 12  # Neutral default

        # 4. Insider signal
        insider_signal = insider.get('signal', 'NEUTRAL') if 'error' not in insider else 'NEUTRAL'
        if insider_signal == 'BULLISH':
            components['insider_signal'] = 25
        elif insider_signal == 'MIXED':
            components['insider_signal'] = 15
        elif insider_signal == 'BEARISH':
            components['insider_signal'] = 5
        else:
            components['insider_signal'] = 12  # NEUTRAL default
