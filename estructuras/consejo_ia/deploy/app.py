...
_layout_dir = os.environ.get(
    "GREYBARK_LAYOUT_DIR",
    str(Path(__file__).resolve().parents[2] / "layout")  # ../../layout if running dev
)
...
def _get_client_reports(client_id: str, platform: Platform) -> list:
    ...
    if not client_dir.exists():
        client_dir = _deploy_dir.parent / "output" / "reports"

    if not client_dir.exists():
        return reports

    for html_file in sorted(client_dir.rglob("*.html"), key=lambda p: p.stat().st_mtime, reverse=True):
        stat = html_file.stat()
        ...
        reports.append({
            "name": html_file.name,
            "path": str(html_file),
            "rel_path": str(html_file.relative_to(client_dir)),
            "size": stat.st_size,
...
async def status_page(
    ...
    platform = _get_platform()
    ctx = _client_context(request, client_id, platform, job=job)
...
async def client_logo(
    client_id: str,
    _current: str = Depends(get_current_client),
):
    platform = _get_platform()
    branding = platform.get_branding(client_id)
    if branding.logo_path and os.path.exists(branding.logo_path):
        return FileResponse(branding.logo_path)
    return RedirectResponse(url="/static/style.css", status_code=303)
