...
class CreditRiskMonitor(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        lines = [
            "[CREDIT RISK MONITOR MODULE]",
            f"Credit Stress Score: {r['stress_score']:.0f}/100 ({r['stress_zone']})",
            f"Investment Grade: {r['ig_regime']} (total {r['ig_bps']:.0f}bps, {r['ig_pctl']:.0f}th percentile)",
        ]
        ...
        lines.append(
            f"High Yield: {r['hy_regime']} (total {r['hy_bps']:.0f}bps, {r['hy_pctl']:.0f}th percentile)"
        )
        ...
        lines.append(f"Quality rotation: {r['quality_signal']}" +
                      (f" ({r['quality_rationale']})" if r.get('quality_rationale') else ''))

        alerts = r.get('widening_alerts', [])
        lines.append(f"Widening alerts: {', '.join(alerts) if alerts else 'None'}")

        # Key risk from interpretation
        interp = r.get('interpretation', '')
        if interp:
            lines.append(f"Key risk: {interp.split('. ')[-1] if '. ' in interp else interp}")
