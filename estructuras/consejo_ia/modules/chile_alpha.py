...
class ChileAlphaSignal(AnalyticsModuleBase):
    ...
    def get_council_input(self) -> str:
        ...
        r = self._result
        lines = [
            "[CHILE ALPHA SIGNAL MODULE]",
            f"Composite Score: {r['composite_score']:+.0f}/100 ({r['zone']})",
            f"Sub-signals ({r['n_signals']} active):",
        ]
        for key, sig in r.get('signals', {}).items():
            arrow = '+' if sig['score'] > 0 else ('-' if sig['score'] < 0 else '=')
            lines.append(f"  {sig['label']} -> {arrow}{sig['score']} (weight {self.WEIGHTS.get(key, 0):.0%})")
