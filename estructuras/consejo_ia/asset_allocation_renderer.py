    ...
    def _render_template(self, template, content: Dict) -> str:
        ...
        for key, placeholder in [('renta_variable', 'dashboard_rv_rows'),
            ...
            for item in dashboard.get(key, []):
                view_class = f"dash-{item['view'].lower()}"
        ...
        eq_rows = ''.join([
            f"<tr><td>{r['region']}</td><td><span class='view-badge badge-{r['view'].lower()}'>{r['view']}</span></td><td>{r['rationale']}</td></tr>"
        ...
        rf_rows = ''.join([
            f"<tr><td>{c['tramo']}</td><td><span class='view-badge badge-{c['view'].lower()}'>{c['view']}</span></td><td>{c['rationale']}</td></tr>"
            for c in rf['curva']
        ])
        ...
        for cat_key, cat_title in category_titles.items():
            ...
            focus_html += f'''<div class="focus-category">
                ...
                <table class="focus-table">
                    ...
                    <tbody>'''
            for item in items:
                badge_class = f"badge-{item['view'].lower()}"
                focus_html += f'''<tr>
